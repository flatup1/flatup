...
## 2026-05-19 参考資料をGit管理し、アーカイブ本文は管理外にした

### 起きたこと

- `99_REFERENCE/` の大型Markdown参考資料は、秘密情報パターンに引っかからず、検索性のためGitHub管理する価値があった。
- `5_アーカイブ/` の本文は、旧手順・未分類メモ・APIキー例などが混ざる可能性があり、一括管理するとノイズが増える状態だった。

### 学んだこと

- 参考資料はGitHub管理してよいが、現行正本ではないと明記する必要がある。
- アーカイブはREADMEだけ管理し、本文はローカル保管にすると `git status` が読みやすくなる。
- 「探せる」と「実行してよい」は別物。

### 次からのルール

- `99_REFERENCE/*.md` は背景資料としてGitHub管理する。
- `5_アーカイブ/**` は `.gitignore` で管理外にし、`5_アーカイブ/README.md` だけ管理する。
- 参考資料やアーカイブ本文から使える知見を見つけたら、正本へ要約して昇格する。
