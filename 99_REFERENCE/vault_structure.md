...
`99_REFERENCE/` は、現行ルールではなく背景資料です。

- 現行仕様は `00_CORE/`、`01_SKILLS_CUSTOMER/`、`02_SKILLS_MARKETING/`、`03_SKILLS_INTERNAL/`、`6_システム/` を優先する。
- 参考資料から使える知見を見つけたら、そのまま実行せず、該当する正本ファイルへ要約して昇格する。
- 古い対話ログや要件定義は、現在の本番状態と矛盾する可能性がある。
...
| `99_REFERENCE/README.md` | 参考資料フォルダの入口 |

## Git管理方針

- `99_REFERENCE/` のMarkdown参考資料は検索性を優先してGitHub管理する。
- `5_アーカイブ/` はREADMEだけをGitHub管理し、本文はローカル保管の過去資料として扱う。
- 参考資料やアーカイブ本文を現行化する時は、正本ファイルへ要約して昇格する。
