...
| `00_CORE/ブランド理念/FLATUP_mission.md` | ルート旧ファイルの正本移動先として採用 |
| `00_CORE/ブランド理念/SOUL.md` | ルート旧ファイルの正本移動先として採用 |
| `00_CORE/ブランド理念/中核コピー.md` | FLATUPの言葉の核として採用 |
| `01_SKILLS_CUSTOMER/README.md` | 顧客対応スキルの入口 |
| `01_SKILLS_CUSTOMER/line_reply.md` | LINE/DM返信の基本テンプレート |
| `01_SKILLS_CUSTOMER/trial_booking.md` | 体験予約案内の型 |
| `01_SKILLS_CUSTOMER/【FLATUP AI OS】成約特化型ナレッジFAQリスト.md` | 顧客対応FAQとして採用 |
| `01_SKILLS_CUSTOMER/FLATUPGYM 成約率最大化マスターガイド：体験設計とLINE接客アルゴリズム.md` | 成約率改善の設計資料として採用 |
| `02_SKILLS_MARKETING/README.md` | マーケティング領域の入口 |
| `02_SKILLS_MARKETING/SNS/Typefully_X運用メモ_FLATUP.md` | X/Typefully運用メモとして採用 |
| `02_SKILLS_MARKETING/SNS/FLATUP_CM_Prompt 2.md` | SNS/CMプロンプトとして採用 |
| `02_SKILLS_MARKETING/SNS/FLATUP GYM ブランド構築・SNS戦略 対話履歴.md` | ブランド戦略の判断材料として採用 |
| `02_SKILLS_MARKETING/LINE広告/*.md` | LINE広告運用資料として採用 |
| `02_SKILLS_MARKETING/配信/*.md` | LINE配信手順とテンプレートとして採用 |
| `02_SKILLS_MARKETING/video_factory/**/*.md` | 動画制作ワークフローとして採用 |
| `02_SKILLS_MARKETING/動画_SORA/*.md` | AI動画プロンプトとして採用 |
| `03_SKILLS_INTERNAL/jin_copilot_protocol.md` | JIN Copilotの行動ルール |
| `03_SKILLS_INTERNAL/task_management/SKILL.md` | タスク管理スキルの入口として採用 |
| `4_日記/JIN_AI_INBOX.md` | JINの未整理メモ |
| `4_日記/JIN_AI_RESUME.md` | 作業再開メモ |
| `4_日記/JIN_AI_TASKS.md` | 整理済みタスク |
| `.claude/commands/jin-capture.md` | JINメモ預かりコマンドとして採用 |
| `.claude/commands/jin-organize.md` | JINタスク整理コマンドとして採用 |
| `.claude/commands/jin-resume.md` | JIN作業再開コマンドとして採用 |
| `6_システム/FLATUPGYM_AI_正本マップ.md` | 正本判断の地図として採用 |
| `6_システム/AIKA_LINE_Bot本体コマンド早見表.md` | LINE Bot運用に有用 |
| `6_システム/AIKA_OS_中学生向け使い方.md` | 既存の入門説明として有用 |
| `6_システム/CLOSED_MODE_DESIGN.md` | 休業日・オーナー限定モードの設計として採用 |
| `6_システム/JIN_COPILOT_OS.md` | JIN Copilot設計 |
| `6_システム/JIN_COPILOT_LINE_COMMAND_DESIGN.md` | LINEコマンド設計として採用。ただし本番反映は別レビュー |
| `6_システム/PHASE1_DESIGN.md` | Phase 1設計 |
| `6_システム/PHASE1_NEXT_STEPS.md` | 再開手順として有用 |
| `6_システム/JIN専用：Master Brain & LINE AI 運用マニュアル.md` | AI OS運用マニュアルとして採用 |
| `6_システム/⭐️FLATUP_AI_OS_修復マニュアル.md` | 復旧時の参照資料として採用 |
| `6_システム/開発フロー.md` | 開発・レビュー・本番反映ルールとして採用 |
| `6_システム/check_flatup_aika.sh` | 監視スクリプト控え |
| `6_システム/code/flatup_config.env.example` | env例。秘密情報なしなら有用 |
| `6_システム/code/tests/run.sh` | 本番コード差分のテスト実行入口として採用 |
| `6_システム/code/tests/` | 本番コード差分の安全確認に必要 |
| `6_システム/flatup-research-ai/` | 調査AIのMVP |
| `6_システム/openqlow_drafts/README.md` | OPENQLOW SNS下書き控えの入口として採用 |
| `6_システム/openqlow_logs/README.md` | OPENQLOW生成・承認ログの入口として採用 |
| `99_REFERENCE/README.md` | 参考資料フォルダの入口として採用 |
| `99_REFERENCE/vault_structure.md` | Vault構造の人間用リファレンスとして採用 |
| `99_REFERENCE/AIKA_OS_BRIEFING_20260514.md` | AIKA OS参考ブリーフィングとして採用。ただし現行正本ではない |
| `99_REFERENCE/FLATUPGYM AI構築：要件定義書 & 対話ログ・マスター.md` | FLATUPGYM AI構築背景として採用。ただし現行正本ではない |
| `99_REFERENCE/🚀 JIN専用：AIビジネス自動化プロジェクト 全要件定義 & 対話ログ・マスター.md` | AI自動化構想の背景資料として採用。ただし現行正本ではない |
| `99_REFERENCE/ダヴィンチ指示文_経営全体最適化.md` | 経営全体最適化プロンプトの参考として採用。ただし現行正本ではない |
...
| `99_REFERENCE/*.md` の大型要件定義・対話ログ | 背景資料として残す。現行ルールにはしない |
...
| `.claude/settings.json` | ローカル権限設定。コマンド本体とは分けてGitHub管理対象外 |
